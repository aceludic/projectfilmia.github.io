...
// --- Journal Content (from former JournalPage.tsx) ---
...
const JournalContent: React.FC<SocialHubPageProps> = ({ entries, onAdd, onUpdate, onRemove }) => {
        ...
        <div className="animate-fade-in">
            ...
            {entries.length === 0 ? (
                 <div className="text-center py-20">
                    <div className="max-w-lg mx-auto liquid-glass p-12 rounded-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {entries.map((entry, index) => (
                        <div key={entry.id} className="liquid-glass p-5 flex flex-col animate-fade-in-up transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5 hover:scale-[1.03]" style={{ animationDelay: `${index * 50}ms` }}>
                            ...
                            <div className="flex justify-end space-x-2 border-t border-glass-border dark:border-glass-border-dark pt-3 mt-auto">
                                ...
                                <button onClick={() => handleDelete(entry.id)} className="text-xs font-bold text-red-500 hover:underline">Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
...
// --- Community Content (from former CommunityPage.tsx) ---

const communityLinks = [
    {
        name: 'Discord Server',
        description: 'Our official Discord server is coming soon! Join to chat with other students, share resources, and participate in group study sessions.',
        url: '#',
        icon: '💬',
        cta: 'Coming Soon',
        disabled: true,
    ...
    {
        name: 'Social Media',
        description: 'Follow our development progress and get updates on our social media channels. Links will be available here soon!',
        url: '#',
        icon: '📢',
        cta: 'Coming Soon',
        disabled: true,
    },
    {
        ...
        description: "Have a suggestion or found a bug? We'd love to hear from you! Your feedback helps us improve Project Filmia for everyone.",
        url: 'https://docs.google.com/forms/d/e/1FAIpQLSctftbiSZdIgzk3YXlsMaEOh1Q1sI70nQ5DiKbK2I3Qd19uHw/viewform?usp=header',
        icon: '📝',
        cta: 'Open Form',
        disabled: false,
    }
...
const CommunityContent: React.FC = () => (
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 animate-fade-in">
        {communityLinks.map((link, index) => (
            <div key={index} className="liquid-glass rounded-2xl p-8 flex flex-col items-center text-center transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5 hover:scale-[1.03]">
                <div className="text-6xl mb-4">{link.icon}</div>
                <h2 className="text-2xl font-bold text-stone-800 dark:text-beige-100">{link.name}</h2>
                <p className="text-sm text-stone-600 dark:text-stone-400 mt-2 flex-grow">{link.description}</p>
                <a 
                    href={link.url} 
                    target={!link.disabled ? "_blank" : "_self"}
                    rel="noopener noreferrer" 
                    onClick={link.disabled ? (e) => e.preventDefault() : undefined}
                    className={`mt-6 w-full max-w-xs block px-6 py-3 text-white font-bold rounded-lg btn-ripple transition-colors ${link.disabled ? 'bg-stone-400 cursor-not-allowed' : 'bg-brand-brown-700 hover:bg-brand-brown-800'}`}
                >
                    {link.cta}
                </a>
            </div>
...
const SocialHubPage: React.FC<SocialHubPageProps> = (props) => {
    const [activeTab, setActiveTab] = useState<'journal' | 'community'>('journal');

    const TabButton: React.FC<{ tab: 'journal' | 'community', children: string }> = ({ tab, children }) => (
        <button onClick={() => setActiveTab(tab)} className={`flex-1 py-3 text-base font-bold rounded-md transition-colors ${activeTab === tab ? 'bg-brand-brown-700 text-white' : 'text-stone-700 dark:text-stone-300'}`}>
            {children}
        </button>
    );
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="text-center mb-12">
                <h1 className="text-4xl font-black uppercase text-glow">Social Hub</h1>
                <p className="mt-2 text-lg text-stone-500 dark:text-stone-400 max-w-2xl mx-auto">Your personal media log and the community connection point.</p>
            </div>
            
            <div className="max-w-md mx-auto liquid-glass p-2 mb-12 flex items-center space-x-2 rounded-lg">
                <TabButton tab="journal">My Journal</TabButton>
                <TabButton tab="community">Community</TabButton>
            </div>
            
            {activeTab === 'journal' && <JournalContent {...props} />}
